...
L.Map.include({
	...
	_setFreemiumProps: function(freemiumInfo) {
		...
		this.Freemium.freemiumBlacklist = freemiumInfo['FreemiumBlacklist'];
	...
	disableFreemiumItem: function(item, DOMParentElement, buttonToDisable) {
		if (this.isFreemiumUser() && this.isFreemiumBlacklistedItem(item)) {
	...
	isFreemiumBlacklistedItem: function(item) {
		if (this.Freemium.freemiumBlacklist.includes(item.command) // in notebookbar uno commands are stored as command
		|| this.Freemium.freemiumBlacklist.includes(item.uno) // in classic mode uno commands are stored as uno in menus
		|| this.Freemium.freemiumBlacklist.includes(item.id))
			return true;
		return false;
	},
